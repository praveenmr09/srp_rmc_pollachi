...
class SalaryAdvancePayment(models.Model):
    ...
    journal = fields.Many2one('account.journal', string='Journal', domain=[('type', 'in', ['cash', 'bank'])])
    ...
    @api.onchange('employee_id')
    def employee_advance_info(self):
        for rec in self:
            if rec.employee_id:
    ...
    def approve_request(self):
        ...
        # if not address.id:
        #     raise UserError('Define home address for the employee. i.e address under private information of the '
        #                     'employee.')
    ...
    def approve_request_acc_dept(self):
        ...
        for request in self:
            ...
            draft.action_post()
            self.state = 'approve'
            self.employee_id.contract_id.advance_salary = advance_salary
            return True
